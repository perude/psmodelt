 ...
 */
package pl.sap.customsite.test.setup;
...
import pl.sap.customsite.test.constants.CustomsiteTestConstants;
import pl.sap.customsite.test.orders.AcceleratorTestOrderData;
 ...
 */
@SystemSetup(extension = CustomsiteTestConstants.EXTENSIONNAME)
public class TestDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		if (getBooleanSystemSetupParameter(context, CREATE_TEST_DATA))
		{
			...
			importImpexFile(context, "/customsitetest/import/qa-test-users.impex");

			LOG.info("Creating Reviews...");
			importImpexFile(context, "/customsitetest/import/reviews.impex");

			LOG.info("Creating Test Payment Subscriptions...");
			getAcceleratorTestOrderData().createPaymentInfos();

			LOG.info("Creating CS Tickets...");
			importImpexFile(context, "/customsitetest/import/cstickets.impex");
			...
			importImpexFile(context, "/customsitetest/import/csquotes.impex");
		}
	}
}
